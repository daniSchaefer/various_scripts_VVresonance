...
def plotStack(backgrounds,xrange,yrange,zrange,projection,purity):
    ...
    for i in range(len(backgrounds)):
        #if labels[len(backgrounds)-1-i].find("Z")!=-1:
        leg.AddEntry(projections[len(backgrounds)-1-i],labels[len(backgrounds)-1-i],"f")
    
    addText = ROOT.TLatex()
    addText.SetTextFont(42)
    label_proj_2 = str(yrange[0][1])+" < m_{jet2} < "+str(yrange[0][2])+" GeV"
    label_proj_1 = str(xrange[0][1])+" < m_{jet1} < "+str(xrange[0][2])+" GeV"
    ...
    if projection.find("z")==-1 and purity.find("HPHP")!=-1:
        print "set range from 0 to 40"
    if purity.find("VH_HPHP")!=-1 or purity.find("VH_LPHP")!=-1:
        stack.SetMaximum(300)
    else: stack.SetMaximum(150)
    leg.Draw("same")
    if projection=="z":
        c.SetLogy()
        addText.DrawLatexNDC(0.55,0.65,purity)
        addText.DrawLatexNDC(0.55,0.6,"#scale[0.8]{"+label_proj_1+"}")
        addText.DrawLatexNDC(0.55,0.55,"#scale[0.8]{"+label_proj_2+"}")
        stack.GetXaxis().SetTitle("m_{jj} [GeV]")
        stack.GetYaxis().SetTitle("Events / 100 GeV")
        stack.SetMaximum(10e2)
        ...
        if projection =="y":
            ...
            addText.DrawLatexNDC(0.55,0.65,purity)
            addText.DrawLatexNDC(0.55,0.6,"#scale[0.8]{"+label_proj_1+"}")
            addText.DrawLatexNDC(0.55,0.55,"#scale[0.8]{"+label_proj_3+"}")
...
if __name__=="__main__":
    ...
    purity = sys.argv[2]
    f_kernel = ROOT.TFile(directory+"save_new_shapes_2016_pythia_"+purity+"_3D.root","READ")
    f_sample = ROOT.TFile(directory+"JJ_2016_nonRes_"+purity+".root","READ")
    print f_kernel
    print f_sample  
   ...
   # ############### make controlplots of QCD alternative shapes ####################
    ...
    #purity = sys.argv[1]
    f_Wjets  = ROOT.TFile(directory+"JJ_2016_WJets_"+purity+".root","READ")
    f_Zjets  = ROOT.TFile(directory+"JJ_2016_ZJets_"+purity+".root","READ")
    f_ttjets = ROOT.TFile(directory+"JJ_2016_TTJets_"+purity+".root","READ")
    ...
    allevents = qcd.Integral()+ Wjets.Integral() + Zjets.Integral()+ ttbar.Integral()
    print "contribution      events           events/all events "
    print "qcd    "+str(round(qcd.Integral(),1))+"    " + str(round(qcd.Integral()/allevents,1))
    print "Wjets "+str(round(Wjets.Integral(),1)) +"    " + str(round(Wjets.Integral()/allevents,3))
    print "Zjets "+str(round(Zjets.Integral(),1)) +"    " + str(round(Zjets.Integral()/allevents,3))
    print "ttbar "+str(round(ttbar.Integral(),1)) +"    " + str(round(ttbar.Integral()/allevents,3)) 
